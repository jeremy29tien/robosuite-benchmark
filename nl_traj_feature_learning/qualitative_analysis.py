...
def add_embeddings(model, device, trajectories, reference_traj, nl_embedding):
    ...
    with torch.no_grad():
        for i in range(trajectories.shape[0]):
            traj = torch.as_tensor(trajectories[i, :, :], dtype=torch.float32, device=device)
            ...
            print("encoded_target_traj:", encoded_target_traj.shape)
            print("encoded_traj:", encoded_traj.shape)
            dot_prod = torch.dot(encoded_target_traj, encoded_traj)
